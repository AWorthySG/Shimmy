"use client";

import { AnimateOnScroll } from "@/components/animate-on-scroll";
import { useI18n } from "@/lib/i18n";

export default function ContactPage() {
  const { t } = useI18n();

  return (
    <>
      {/* Header */}
      <section className="bg-gradient-to-b from-cream-dark to-cream rice-paper py-14 sm:py-20 px-4 sm:px-6 text-center">
        <div className="mx-auto max-w-3xl">
          <div className="mx-auto h-[2px] w-[60px] bg-gradient-to-r from-transparent via-vermillion/60 to-transparent mb-4" />
          <p className="text-[10px] sm:text-xs uppercase tracking-[0.35em] text-vermillion-dark">
            {t("contact.tag")}
          </p>
          <h1 className="mt-4 font-serif text-3xl sm:text-4xl text-charcoal md:text-5xl shimmer-text">
            {t("contact.title")}
          </h1>
          <p className="mt-3 sm:mt-4 text-sm sm:text-base text-charcoal-light">
            {t("contact.desc")}
          </p>
        </div>
      </section>

      {/* Contact content */}
      <section className="bg-soft-white py-14 sm:py-20 px-4 sm:px-6">
        <div className="mx-auto grid max-w-5xl gap-10 sm:gap-12 md:grid-cols-2">
          {/* Booking CTAs */}
          <AnimateOnScroll animation="fade-right">
          <div>
            <div className="h-[2px] w-[60px] bg-gradient-to-r from-vermillion/60 to-transparent mb-4" />
            <h2 className="font-serif text-xl sm:text-2xl text-charcoal md:text-3xl">
              {t("contact.book.title.1")}
              <br />
              {t("contact.book.title.2")}
            </h2>
            <p className="mt-3 sm:mt-4 text-sm leading-relaxed text-charcoal-light">
              {t("contact.book.desc")}
            </p>

            <div className="mt-6 sm:mt-8 space-y-3 sm:space-y-4">
              {/* TODO: Replace with your WhatsApp number (format: country code + number, no spaces/dashes) */}
              <a
                href="https://wa.me/6589308973"
                target="_blank"
                rel="noopener noreferrer"
                className="group flex items-center gap-3 sm:gap-4 border border-vermillion/20 bg-cream/50 p-4 sm:p-5 transition-all duration-300 hover:border-vermillion/50 hover:shadow-lg hover:shadow-vermillion/5 touch-target shine-on-hover border-glow"
              >
                <div className="flex h-10 w-10 sm:h-12 sm:w-12 shrink-0 items-center justify-center bg-vermillion/5 text-vermillion">
                  <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                  </svg>
                </div>
                <div className="min-w-0">
                  <p className="text-sm font-medium text-charcoal">{t("contact.wa.label")}</p>
                  {/* TODO: Replace with your number */}
                  <p className="text-xs text-charcoal-light">
                    {t("contact.wa.desc")}
                  </p>
                </div>
                <span className="ml-auto text-vermillion opacity-0 transition-opacity group-hover:opacity-100 shrink-0">
                  →
                </span>
              </a>

              {/* TODO: Replace with your Instagram handle */}
              <a
                href="https://instagram.com/shimmyhands.shop"
                target="_blank"
                rel="noopener noreferrer"
                className="group flex items-center gap-3 sm:gap-4 border border-vermillion/20 bg-cream/50 p-4 sm:p-5 transition-all duration-300 hover:border-vermillion/50 hover:shadow-lg hover:shadow-vermillion/5 touch-target shine-on-hover border-glow"
              >
                <div className="flex h-10 w-10 sm:h-12 sm:w-12 shrink-0 items-center justify-center bg-vermillion/5 text-vermillion">
                  <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
                  </svg>
                </div>
                <div className="min-w-0">
                  <p className="text-sm font-medium text-charcoal">{t("contact.ig.label")}</p>
                  {/* TODO: Replace with your handle */}
                  <p className="text-xs text-charcoal-light">
                    {t("contact.ig.desc")}
                  </p>
                </div>
                <span className="ml-auto text-vermillion opacity-0 transition-opacity group-hover:opacity-100 shrink-0">
                  →
                </span>
              </a>

              {/* TODO: Replace with real Google Business review link */}
              <a
                href="https://g.page/shimmyhands/review"
                target="_blank"
                rel="noopener noreferrer"
                className="group flex items-center gap-3 sm:gap-4 border border-vermillion/20 bg-cream/50 p-4 sm:p-5 transition-all duration-300 hover:border-vermillion/50 hover:shadow-lg hover:shadow-vermillion/5 touch-target shine-on-hover border-glow"
              >
                <div className="flex h-10 w-10 sm:h-12 sm:w-12 shrink-0 items-center justify-center bg-vermillion/5 text-vermillion">
                  <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M12.48 10.92v3.28h7.84c-.24 1.84-.853 3.187-1.787 4.133-1.147 1.147-2.933 2.4-6.053 2.4-4.827 0-8.6-3.893-8.6-8.72s3.773-8.72 8.6-8.72c2.6 0 4.507 1.027 5.907 2.347l2.307-2.307C18.747 1.44 16.133 0 12.48 0 5.867 0 .307 5.387.307 12s5.56 12 12.173 12c3.573 0 6.267-1.173 8.373-3.36 2.16-2.16 2.84-5.213 2.84-7.667 0-.76-.053-1.467-.173-2.053H12.48z" />
                  </svg>
                </div>
                <div className="min-w-0">
                  <p className="text-sm font-medium text-charcoal">{t("contact.google.label")}</p>
                  <p className="text-xs text-charcoal-light">
                    {t("contact.google.desc")}
                  </p>
                </div>
                <span className="ml-auto text-vermillion opacity-0 transition-opacity group-hover:opacity-100 shrink-0">
                  →
                </span>
              </a>
            </div>

            {/* TODO: Update your operating hours */}
            <div className="mt-8 sm:mt-10 border-t border-vermillion/10 pt-6 sm:pt-8">
              <h3 className="text-[10px] sm:text-xs uppercase tracking-[0.2em] text-vermillion-dark mb-4">
                {t("contact.hours.tag")}
              </h3>
              <ul className="space-y-2 text-sm text-charcoal-light">
                <li className="flex justify-between gap-4">
                  <span>{t("contact.hours.mf")}</span>
                  <span className="text-right">{t("contact.hours.mf.time")}</span>
                </li>
                <li className="flex justify-between gap-4">
                  <span>{t("contact.hours.sat")}</span>
                  <span className="text-right">{t("contact.hours.sat.time")}</span>
                </li>
                <li className="flex justify-between gap-4">
                  <span>{t("contact.hours.sun")}</span>
                  <span className="text-right">{t("contact.hours.sun.time")}</span>
                </li>
              </ul>
            </div>
          </div>
          </AnimateOnScroll>

          {/* Map / Location */}
          <AnimateOnScroll animation="fade-left" delay={150}>
          <div>
            <div className="h-[2px] w-[60px] bg-gradient-to-r from-vermillion/60 to-transparent mb-4" />
            <h2 className="font-serif text-xl sm:text-2xl text-charcoal md:text-3xl">
              {t("contact.visit.title")}
            </h2>
            <p className="mt-3 sm:mt-4 text-sm leading-relaxed text-charcoal-light">
              {t("contact.visit.desc")}
            </p>

            {/* ┌──────────────────────────────────────┐
                │  TODO: Replace with Google Maps embed │
                │  <iframe                              │
                │    src="https://maps.google.com/..."  │
                │    className="w-full h-full border-0" │
                │    loading="lazy" />                  │
                └──────────────────────────────────────┘ */}
            <div className="mt-6 sm:mt-8 aspect-[4/3] border border-vermillion/10 bg-gradient-to-br from-cream-dark to-jade/5 flex flex-col items-center justify-center">
              <span className="text-4xl sm:text-5xl text-vermillion/20">📍</span>
              <p className="mt-4 text-[10px] sm:text-xs uppercase tracking-[0.2em] text-warm-gray">
                Map Embed Here
              </p>
              {/* TODO: Replace with your actual address */}
              <p className="mt-2 text-sm text-charcoal-light">
                {t("contact.visit.address.1")}
              </p>
              <p className="text-sm text-charcoal-light">{t("contact.visit.address.2")}</p>
            </div>

            {/* Pre-visit info */}
            <div className="mt-6 sm:mt-8 oriental-corner border border-vermillion/15 bg-cream/30 p-5 sm:p-6 border-glow">
              <h3 className="text-[10px] sm:text-xs uppercase tracking-[0.2em] text-vermillion-dark mb-3">
                {t("contact.prep.tag")}
              </h3>
              {/* TODO: Update pre-visit instructions */}
              <ul className="space-y-2 text-sm text-charcoal-light">
                <li className="flex items-start gap-2">
                  <span className="mt-0.5 text-vermillion text-xs shrink-0">◆</span>
                  {t("contact.prep.1")}
                </li>
                <li className="flex items-start gap-2">
                  <span className="mt-0.5 text-vermillion text-xs shrink-0">◆</span>
                  {t("contact.prep.2")}
                </li>
                <li className="flex items-start gap-2">
                  <span className="mt-0.5 text-vermillion text-xs shrink-0">◆</span>
                  {t("contact.prep.3")}
                </li>
                <li className="flex items-start gap-2">
                  <span className="mt-0.5 text-vermillion text-xs shrink-0">◆</span>
                  {t("contact.prep.4")}
                </li>
              </ul>
            </div>
          </div>
          </AnimateOnScroll>
        </div>
      </section>
    </>
  );
}
